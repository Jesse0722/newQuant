...
def _enrich_ladder_industry(ladder: list[dict]) -> None:
    """为连板天梯每行补充 industry（来自本地 stock_basic）。"""
    if not ladder:
        return
    codes = [str(r["ts_code"]) for r in ladder if r.get("ts_code")]
    if not codes:
        for row in ladder:
            row["industry"] = None
        return
    ...
    try:
        rows = (
            db.query(StockBasic.ts_code, StockBasic.industry)
...
def get_limit_market_board_payload(
    ...
    try:
        _enrich_ladder_industry(ladder)
    except Exception:
        for row in ladder:
