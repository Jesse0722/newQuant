...
class MainWaveScreenRequest(BaseModel):
    ...
    statuses: list[str] = Field(
        default_factory=lambda: ["main_wave_confirmed", "breakout_tracking", "watching"]
    )
    ...
    min_sector_return_20d: Optional[float] = Field(None)
    min_relative_strength_20d: Optional[float] = Field(None)
    entry_stages: list[str] = Field(default_factory=list, description="买点阶段过滤，为空则不过滤")
    min_entry_score: Optional[int] = Field(None, ge=0, le=100)
    exclude_overheat: bool = True


class MainWaveBacktestRequest(MainWaveScreenRequest):
    trade_date_from: str = Field(..., description="开始日期 YYYYMMDD")
    trade_date_to: str = Field(..., description="结束日期 YYYYMMDD")
    holding_days: list[int] = Field(default_factory=lambda: [1, 3, 5, 10], max_length=4)
    max_signals_per_day: int = Field(20, ge=1, le=100)
    cooldown_days: int = Field(5, ge=0, le=30)
