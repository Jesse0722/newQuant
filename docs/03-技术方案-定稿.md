...
| 方法 | 路径 | 说明 | 响应 |
|------|------|------|------|
| GET | `/api/dashboard` | 涨停最强板块 + 连板天梯（Tushare `limit_cpt_list` / `limit_step`） | 见下 |

**Query：** `trade_date`（可选，`YYYYMMDD`）— 指定数据日；缺省时由服务端解析「最近已完成交易日」。
...
```json
{
  ...
  "ladder": [
    { "ts_code": "000833.SZ", "name": "粤桂股份", "trade_date": "20250328", "nums": "11" }
  ]
}
```

**错误：** `detail` 可为对象 `{ "code", "message", "hint" }`（如 Tushare 不可用、积分不足、交易日解析失败）；前端整页错误态展示，不使用假数据。
