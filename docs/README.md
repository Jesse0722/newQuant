...
| 模块 | 说明 |
|------|------|
| 观察池 | 多分组、单个添加 + CSV 批量导入、池级默认监控条件 |
| 数据同步 | 仅观察池中股票、Tushare Pro、增量同步、加入时自动拉 250 天 |
| 监控提醒 | 6 个预置模板 + 自定义组合、同步后自动扫描 + 手动、系统内提醒 |
| 交易计划 | 单股/计划、价格目标 + 盈亏比、风险等级 1-5 星、计划级复盘 |
| 交易明细 | 手动记录、关联交易计划、自动算金额/印花税、盈亏自动汇总 |
